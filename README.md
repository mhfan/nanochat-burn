...
| 方面 | Python nanochat | nanochat-burn |
|---|---|---|
| 后端与主要目标 | PyTorch/CUDA、FlashAttention、`torch.compile`，面向 H100 speedrun | Burn + WGPU/Metal/Vulkan 或 NdArray，面向可移植实验和 Rust 集成 |
| 模型 | 上游基准定义 | 对齐核心 GPT 数学，并提供 ReLU²、QK Norm、GQA、SWA、Smear、Backout 消融开关和跨语言 fixtures |
| 预训练数据 | Parquet 流式文档、BOS-aligned best-fit packing、真实多 rank 分片 | 本地文本预分词为 mmap u32 shards；现在按空行识别文档、长文档分片，并以 BOS-aligned best-fit 固定行写盘 |
| 训练规模 | 多 GPU optimizer sharding，并有 CUDA FP8 路径 | 当前训练执行仍是单进程；支持可恢复梯度累积，但没有真实 collective 多 GPU 和 FP8 训练 |
| 优化器 | 融合/编译的 AdamW + 分布式 Muon | 泛型 Burn 实现；AdamW 在 f16 模型下仍以 fp32 保存和计算状态，Muon 对齐 MuonEq/Muon+/NorMuon |
| Checkpoint | PyTorch checkpoint，数据位置近似恢复 | 模型、tokenizer、配置、optimizer、trainer、dataloader/RNG 组成统一 safetensors artifact，强调精确恢复 |
| 推理 | 连续 KV cache；同 prompt 先 prefill 一次再扩展样本 | Paged KV、请求页表/回收、attention sinks、continuous batching；同 prompt 只计算一次并复制独立 KV 页 |
| 推理扩展 | 基础生成和当前上游 benchmark | W8/W4 weight-only、greedy speculative decoding、设备采样器、SSE Web 服务和内存/TPOT 基准 |
| 对齐 | SFT 与较精简的 group-relative RL | Packed SFT（含梯度累积）、REINFORCE/GRPO、old-policy clip、reference KL、rollout/RNG 恢复 |
| 产品边界 | 上游主动保持最小，已移除 Web UI、报告和部分教学任务 | 保留报告、Web UI、离线 tiny recipe 和学习文档，作为 Rust 系统实验面的一部分 |

项目侧的 WGPU 元素类型已经集中为 `src/common.rs` 中的 `ModelFloat`，避免 dtype 选择散落到模型、
cache 和 optimizer。FP8 仍不能通过把这个别名中的 `f16` 替换为某个八位类型来立即获得：
Burn 0.21 的普通浮点 Tensor dtype 只有 f64/f32/Flex32/f16/bf16；E4M3/E5M2 目前属于
`QFloat` 量化存储语义，不是可直接参与完整 autograd 训练的 backend float element。真正的 FP8
训练还需要硬件能力检测、带缩放的 GEMM/attention kernel、amax/scale 历史、fp16/bf16 或 fp32
梯度与 master state、溢出策略、checkpoint 元数据和数值/吞吐验证。当前 W8/W4 也只是推理期
weight-only 量化，不能替代这些工作。因此在 Burn 后端提供并验证完整训练原语之前，本项目继续以
f16 forward/backward + fp32 AdamW 状态作为低精度训练路径。
