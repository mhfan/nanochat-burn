...
| W8/W4 weight-only quantization | Experimental | WGPU 使用 Burn QFloat 快路径，Flex 和特殊形状使用可移植回退 |
...
|---|---|---|---|
| 后端与主要目标 | PyTorch/CUDA、FlashAttention、`torch.compile`，面向 H100 speedrun | Burn + WGPU/Metal/Vulkan 或 Flex，面向可移植实验和 Rust 集成 | Python：训练性能；Burn：可移植性 |
...
- **低比特部署实验**：比较 f16、W8A16、W4A16 的 logits 误差、模型字节数和真实延迟，筛选
  适合本地或资源受限设备的模型配置。
- **优化器与后端回归平台**：利用同一 Python fixture、Flex reference 和 WGPU 路径验证新
...
*   **系统级优化**：
    ...
    *   **设备切换**：默认使用可移植的 Flex CPU 后端；训练、推理和 GPU 基准通过 `--no-default-features --features wgpu` 显式启用 WGPU。
...
```text
    ...
    └── src/
        ...
        └── bin/
            ...
            ├── bench/          # 统一 benchmark 命令
            │   ├── main.rs     # infer/speculative/ops 子命令分发
            │   ├── infer.rs    # Prefill/decode 与量化基准
            │   ├── speculative.rs # 推测解码 acceptance/speedup 基准
            │   └── ops.rs      # RMSNorm、RoPE、Softmax 与 attention 算子基准
...
### 1. 运行单元测试
单元测试默认使用 F32 Flex 后端：
```bash
cargo test
```

需要验证 WGPU/f16、原生量化或显存统计时额外启用 WGPU feature；普通测试仍保持 Flex：
...
量化误差。表中实测值来自当前 Flex 与 Metal/WGPU 测试环境，预算由测试常量持续执行：

| 路径 | 参照 | 实测误差 | 预算 |
|---|---|---:|---:|
| Flex f32 | Python f32 | 4.8e-7 | 5e-5 |
| WGPU/Metal f16 | Python f32 | 2.63619e-3 | 5e-3 |
| Flex portable W8 | Flex f32 | 7.4208e-4 | 5e-3 |
| WGPU native W8 | WGPU f16 | 2.31934e-3 | 5e-3 |
| Flex portable W4, block 8 | Flex f32 | 2.99752e-3 | 2e-2 |
| WGPU native W4, block 8 | WGPU f16 | 4.15039e-3 | 2e-2 |

一条命令运行 Flex 与 WGPU parity suites，并将测试清单、实测误差、预算、revision
...
无 GPU 环境可使用 `--backend flex`；`--backend wgpu` 可单独验证加速后端，
...
cargo run --release --no-default-features --features wgpu --bin bench -- infer --artifact runs/sft --quantization 4
...
字节数，以及 CubeCL allocator 实测的设备 `bytes_in_use`/`bytes_reserved` 峰值。Flex 没有设备
...
*   **CLI 命令行对话客户端**：
    ...
    ```
    *（使用默认 Flex 在 CPU 验证：`cargo run --release --bin chat`）*
...
1.  **泛型后端抽象**：模型、注意力和优化器基于 `<B: Backend>` 或 `<B: AutodiffBackend>`；当前持续验证 WGPU 与 Flex。
