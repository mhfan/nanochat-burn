...
*   **系统级优化**：
    ...
    *   **设备切换**：默认使用 WGPU，也可通过 `--no-default-features --features ndarray` 进行纯 CPU 快速验证。
...
### 1. 运行单元测试
单元测试固定使用 F32 NdArray 后端，不需要在各测试模块重复声明 feature。若要完全排除
WGPU 编译和设备专项测试，可运行：
```bash
cargo test --no-default-features --features ndarray
```

默认测试还会执行显式标记的 WGPU/f16、原生量化和显存统计测试：
...
实际模型能力。移除 `--no-default-features --features ndarray` 即可使用默认 WGPU 后端运行同一 recipe。
...
NANOCHAT_CONFIG=configs/tiny.toml cargo run --no-default-features --features ndarray --bin eval
