...
This directory contains repository-owned fixtures, the embedded Web UI, and local SFT and
evaluation datasets. Dataset preparation scripts live in `tools/`; generated datasets and download
...
1. **`tools/download_datasets.py`**:
   - **Purpose**: The single acquisition/export entry point. It downloads public raw files for
     ...
     `sft_train.jsonl` and the six `eval/*.jsonl` datasets. Downloads are cached under the ignored
...
2. **`tools/generate_synthetic_datasets.py`**:
...
```bash
uv run --no-project tools/download_datasets.py
```
...
`uv run --no-project tools/generate_synthetic_datasets.py` instead.
