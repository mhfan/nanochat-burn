...
impl<B: Backend, LTarget: ForwardLayer<B>, LDraft: ForwardLayer<B>> SpeculativeInferenceEngine<B, LTarget, LDraft> {
    ...
    pub fn prefill(&self, prompt_tokens: &[usize], device: &B::Device) -> (SpeculativeState<B>, Tensor<B, 2>) {
        let (draft_state, _) = self.draft_engine.prefill(prompt_tokens, 1, device);
        let (target_state, target_logits) = self.target_engine.prefill(prompt_tokens, 1, device);

        let state = SpeculativeState { target_state, draft_state,
            current_tokens: prompt_tokens.to_vec(), step: prompt_tokens.len(),
        };

        (state, target_logits)
    }

    /// Perform speculative decoding steps: draft K tokens, evaluate in parallel, and verify losslessly
    pub fn step_speculative(&self, state: &mut SpeculativeState<B>, target_logits: Tensor<B, 2>,
        ...
        let target_input = Tensor::<B, 2, Int>::from_data(
            TensorData::new(draft_tokens.iter().map(|&t| t as i32).collect::<Vec<_>>(), Shape::new([1, draft_len])),
            device,
        );

        let target_logits_3d = self.target_engine.model.forward_with_cache(
            target_input, &mut state.target_state.cache, state.step,
        );
        ...
        for i in 0..draft_len {
            ...
            // The target prediction for draft_tok (at position L + i) is sampled from target_logits (for i == 0) or target_logits_3d (for i > 0)
            ...
            let target_pred_toks = sample_next_token(
                target_logits_for_verify, temperature, top_k,
                repetition_penalty, &state.target_state.current_tokens,
            );
            ...
            if draft_tok == target_pred_tok {
                ...
                if i == draft_len - 1 {
                    ...
                    accepted_count += 1;
                    final_next_logits = next_target_logits;

                    if last_pred_tok == assistant_end || last_pred_tok == bos { is_finished = true; }
                }
            } else {
                ...
                // Run correction forward pass for target_pred_tok to overwrite cache at position L + i and get next logits
    ...
    pub fn generate(&self, prompt_tokens: &[usize], max_tokens: usize, k_spec: usize,
        ...
        while step_count < max_tokens {
            let (accepted, next_logits, is_finished) = self.step_speculative(&mut state,
                cur_logits, k_spec, temperature, top_k, repetition_penalty, device,
            );
            cur_logits = next_logits;
            step_count += accepted.len();
...
//#[cfg(test)] mod tests { use super::*;
    #[test] fn test_speculative_decoding_lossless() {
        ...
        let corpus = vec!["Rust is extremely elegant, ultra-fast, and lossless speculative decoding works!"];
        ...
        let spec_engine = SpeculativeInferenceEngine::new(target_model.clone(), draft_model.clone(), tokenizer.clone());
        ...
        let (mut target_state, mut target_logits) = target_engine.prefill(&prompt_tokens, 1, &device);
        let mut target_res = prompt_tokens.clone();
        for _ in 0..10 {
            let (toks, _, next_logits) = target_engine.step_generation(&mut target_state, target_logits, 0.0, None, 1.0, &device);
        ...
        assert_eq!(spec_res, target_res, "Speculative decoding did not maintain lossless parity with target model!");
    }
//}
