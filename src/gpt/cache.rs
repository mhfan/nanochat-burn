...
impl<B: Backend> KVCache<B> {
    ...
    pub fn truncate(&mut self, len: usize) {
        ...
        for request in 0..self.batch_size { self.truncate_request(request, len); }
        if let Some(history) = self.token_history.take() {
            let cleared = if len < self.max_seq_len {
                let device = history.device();
                history.slice_assign([0..self.batch_size, len..self.max_seq_len],
                    Tensor::zeros([self.batch_size, self.max_seq_len - len], &device))
            } else { history };
            self.token_history = Some(cleared);
        }
        self.len = len;
    }
    ...
    pub fn evict_for_attention_sinks(&mut self, request: usize, sink_tokens: usize,
        recent_tokens: usize) {
        assert!(request < self.batch_size, "request index exceeds cache batch size");
        let sink_pages = sink_tokens.min(self.len).div_ceil(self.page_size);
        let recent_start = self.len.saturating_sub(recent_tokens) / self.page_size;
    ...
    pub(super) fn update(&mut self, layer_idx: usize, k: Tensor<B, 4>, v: Tensor<B, 4>,
        ...
        self.ensure_pages(start_page, end_page);
        for batch_idx in 0..batch_size {
            for page in start_page..=end_page {
                ...
                let physical_page = self.block_tables[batch_idx][page]
