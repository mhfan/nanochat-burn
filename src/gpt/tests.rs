...
#[test] fn test_gpt_forward_and_loss() {
    ...
    let gpt: Gpt<TrainBackend> = Gpt::new(config, &device);
...
type Int2DModelTensor = Tensor<InferBackend, 2, Int>;

#[test] fn test_burn_attention_paths_match_reference() {
    let device = Default::default();
    let (batch_size, n_head, sequence_len, head_dim) = (2, 4, 32, 16);
    let q = Tensor::<InferBackend, 4>::random([batch_size, n_head, sequence_len, head_dim],
...
#[test] fn test_relu_squared_ablation_changes_mlp_output() {
    let device = Default::default();
    let fc = linear(Tensor::<InferBackend, 2>::from_data(
...
#[test] fn test_cached_forward_matches_full_and_incremental_forward() {
    ...
    let mut gpt: Gpt<InferBackend> = Gpt::new(config, &device);
...
#[test] fn test_paged_attention_roundtrip() {
    ...
    let gpt: Gpt<InferBackend> = Gpt::new(config, &device);
...
#[test] fn test_copied_cache_requests_own_physical_pages() {
    let device = Default::default();
    let mut cache = KVCache::<InferBackend>::new_paged(1, 2, 8, 1, 2, 2, &device);
...
#[test] fn test_attention_sink_eviction_preserves_logical_pages() {
    let device = Default::default();
    let mut cache = KVCache::<InferBackend>::new_paged(1, 1, 16, 1, 8, 2, &device);
